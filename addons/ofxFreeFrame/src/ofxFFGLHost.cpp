...
void ofxFFGLHost::allocate(int w, int h, int internalGlDataType){
    ...
    glTexImage2D(texData.textureTarget, 0, internalGlDataType, texData.tex_w, texData.tex_h, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, 0);  // init to black...
    ...
    t.Handle = (GLuint)texData.textureName[0];
    t.Width = w;
    t.Height = h;  
...
void ofxFFGLHost::draw(float x, float y, float w, float h) {
    texData.textureName[0] = nextTexture.Handle;
